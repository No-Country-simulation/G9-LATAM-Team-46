# Árbol de conocimientos — DataSetTechmindV5

**Proyecto:** TechMind AI · G9-LATAM-Team-46 · Hackathon ONE G9  
**Corpus alineado:** `DataSetTechmindV5.csv` (**1061** filas)  
**Train:** `DataSetTechmindV5_entrenamiento.csv` (**955** filas)  
**Versión del artefacto:** TechmindV5.0 · 2026-08-04  
**Formato:** homólogo a `DataSetTechmindV1.ARBOL.md`  
**Taxonomía de referencia:** primer árbol canónico TechMind  
(`arbol_conocimiento_techmind.md` → consolidado `arbol_conocimiento_v5_completo.md`),  
**podado y expandido** según el contenido real de este CSV.

## Cómo se usa

| Nivel | Rol |
|-------|-----|
| **L1** | Salida del clasificador (`categoria_l1`) |
| **L2–L4** | Vocabulario, keywords, diseño de lotes |
| **BLOQUE / módulos** | Densidad curricular de referencia (no son clases del modelo) |

## Composición del corpus TechmindV5

| Capa | Origen | Filas |
|------|--------|------:|
| PDF nativo / OCR curricular residual | vault + ContieneImagenes (tras exclusiones) | 466 |
| Lotes fundamentales limpios | balanceo/Tavily reescrito estilo base (sin `.op`) | 443 |
| Top-up V5 | plan corte de raíz · opción A ~150/L1 | 152 |
| **Total general** | | **1061** |
| Entrenamiento | filtro longitud ≥ p10 del full | **955** |

```text
DataSetTechmindV4.5 (1190)
  − 281 exclusiones (blacklist PDF + OP_PAGE + flags OCR)
  = base 909
  + 152 top-up técnico (estilo dataset_general_base)
  = DataSetTechmindV5 (1061)
  → train 955 (longitud ≥ p10)
```

### Conteos L1 actuales

| categoria_l1 | Filas | % |
|--------------|------:|--:|
| `Hardware` | 157 | 14.8% |
| `Inteligencia_Artificial` | 153 | 14.4% |
| `Redes_y_Comunicaciones` | 151 | 14.2% |
| `Sistemas_Operativos` | 150 | 14.1% |
| `Lenguajes_Programacion` | 150 | 14.1% |
| `Arquitectura` | 150 | 14.1% |
| `Bases_de_Datos` | 150 | 14.1% |
| **TOTAL** | **1061** | 100% |

**Ratio max/min:** 1.05  
**Longitud:** media 2029 · mediana 2114 · min 526 · max 6396

## Fuentes por capa (derivadas del CSV V5)

Inventario real de `titulo_origen` en `DataSetTechmindV5.csv` (105 orígenes).

### A. PDF nativo / OCR curricular (466 filas)

| Origen (ejemplos) | L1 | n |
|-------------------|----|--:|
| `Conceptos básicos de redes.pdf` | `Redes_y_Comunicaciones` | 92 |
| `Módulo 1 y 2 Fundamentos de Hardware.pdf` | `Hardware` | 61 |
| `Tkinter_Fundamental.pdf` | `Lenguajes_Programacion` | 49 |
| `Java_Fundamental.pdf` | `Lenguajes_Programacion` | 46 |
| `SQL_Fundamental.pdf` | `Bases_de_Datos` | 36 |
| `DatosIBM.pdf` | `Inteligencia_Artificial` | 27 |
| `Fundamentos de Redes y TCP.pdf` | `Redes_y_Comunicaciones` | 26 |
| `IBM IA.pdf` | `Inteligencia_Artificial` | 19 |
| `Curso Python.pdf` | `Lenguajes_Programacion` | 16 |
| `Prompt AWS.pdf` | `Inteligencia_Artificial` | 13 |
| `Guía Técnica de Oracle Virtual Machine.pdf` | `Sistemas_Operativos` | 12 |
| `IBM Data.pdf` | `Inteligencia_Artificial` | 11 |
| `IntroducciónPython_AnálisisDatos.pdf` | `Lenguajes_Programacion` | 11 |
| `Optimizando FMs.pdf` | `Inteligencia_Artificial` | 11 |
| `Ciencia de datos con Python FundamentalsCisco.pdf` | `Inteligencia_Artificial` | 9 |
| `Ciencia de datos IBM.pdf` | `Inteligencia_Artificial` | 8 |
| `VisualizaciónInterpretacióndeDatos.pdf` | `Inteligencia_Artificial` | 8 |
| `Ciencia_de_datos_intro.pdf` | `Inteligencia_Artificial` | 4 |
| `Limpieza y Optimización de Datos en Pandas.pdf` | `Inteligencia_Artificial` | 4 |
| `AWS IA.pdf` | `Inteligencia_Artificial` | 3 |

### B. Lotes fundamentales (pre-top-up, 443 filas)

Incluye balanceo Tavily/subagente ya reescrito a estilo base (sin páginas `.op` residuales).

**`Arquitectura`** (48):
- `Ciberseguridad_Fundamentos_AppSec_Fundamental.pdf` × 26
- `Arquitectura_Microservices_EDA_OCI_Fundamental.pdf` × 8
- `IA_Clasificacion_Texto_RAG_Fundamental.pdf` × 8
- `Arquitectura_Resiliencia_Microservicios_Topup_Fundamental.pdf` × 6

**`Bases_de_Datos`** (105):
- `Bases_Datos_Transacciones_Indices_Fundamental.pdf` × 33
- `Bases_Datos_Transacciones_ACID_Isolation_Fundamental.pdf` × 22
- `Bases_Datos_Indices_Optimizador_Planes_Fundamental.pdf` × 16
- `Bases_Datos_CAP_Replicacion_Particionado_Fundamental.pdf` × 14
- `Bases_Datos_NoSQL_Familias_LSM_Fundamental.pdf` × 12
- `Bases_Datos_CAP_Cassandra_Fundamental.pdf` × 8

**`Hardware`** (96):
- `Hardware_Arquitectura_Computadores_Fundamental.pdf` × 44
- `Hardware_Procesadores_CPU_Pipeline_Fundamental.pdf` × 19
- `Hardware_Almacenamiento_NVMe_Buses_PCIe_Fundamental.pdf` × 17
- `Hardware_Jerarquia_Memoria_MMU_TLB_Fundamental.pdf` × 16

**`Inteligencia_Artificial`** (36):
- `IA_Clasificacion_Texto_RAG_Fundamental.pdf` × 31
- `IA_Topup_Fundamentos_Operativos_Fundamental.pdf` × 5

**`Lenguajes_Programacion`** (16):
- `Lenguajes_Java_Concurrency_Python_Asyncio_Fundamental.pdf` × 16

**`Redes_y_Comunicaciones`** (33):
- `Redes_QUIC_TCP_Congestion_gRPC_Fundamental.pdf` × 22
- `Ciberseguridad_Fundamentos_AppSec_Fundamental.pdf` × 11

**`Sistemas_Operativos`** (109):
- `Sistemas_Operativos_Memoria_Virtual_PageCache_Fundamental.pdf` × 26
- `Sistemas_Operativos_Planificacion_CFS_Concurrencia_Fundamental.pdf` × 25
- `Sistemas_Operativos_Namespaces_Cgroups_Contenedores_Fundamental.pdf` × 18
- `Sistemas_Operativos_IO_BlockLayer_Filesystems_Fundamental.pdf` × 16
- `Sistemas_Operativos_Memoria_IO_Contenedores_Fundamental.pdf` × 14
- `Sistemas_Operativos_CFS_OSTEP_Fundamental.pdf` × 10

### C. Top-up V5 (152 filas)

Generado en el plan V5 (opción A ~150/L1): prosa técnica estilo `dataset_general_base`, **sin** densify y **sin** `.op`.

**`Arquitectura`** (+102):
- `Arquitectura_API_Contratos_Fundamental.pdf` × 3
- `Arquitectura_Patrones_Outbox_Fundamental.pdf` × 3
- `Arquitectura_Observabilidad_SLI_SLO_Fundamental.pdf` × 3
- `Arquitectura_Observabilidad_Tracing_Fundamental.pdf` × 3
- `Arquitectura_Patrones_BFF_Fundamental.pdf` × 3
- `Arquitectura_Patrones_CQRS_Fundamental.pdf` × 3
- `Arquitectura_Patrones_Event_Driven_Fundamental.pdf` × 3
- `Arquitectura_Patrones_Hexagonal_Fundamental.pdf` × 3
- `Arquitectura_Patrones_Saga_Fundamental.pdf` × 3
- `Arquitectura_API_Idempotencia_Fundamental.pdf` × 3
- `Arquitectura_Patrones_Strangler_Fundamental.pdf` × 3
- `Arquitectura_Resiliencia_Bulkhead_Fundamental.pdf` × 3
- `Arquitectura_Resiliencia_Circuit_Breaker_Fundamental.pdf` × 3
- `Arquitectura_Resiliencia_Rate_Limit_Fundamental.pdf` × 3
- `Arquitectura_Resiliencia_Retry_Backoff_Fundamental.pdf` × 3
- `Arquitectura_Resiliencia_Timeout_Deadline_Fundamental.pdf` × 3
- `Arquitectura_Observabilidad_Metrics_Fundamental.pdf` × 3
- `Arquitectura_Micro_vs_Modular_Fundamental.pdf` × 3
- `Arquitectura_Dominio_DDD_Contextos_Fundamental.pdf` × 3
- `Arquitectura_Dominio_Aggregates_Fundamental.pdf` × 3
- `Arquitectura_API_Paginacion_Fundamental.pdf` × 3
- `Arquitectura_API_Versionado_Fundamental.pdf` × 3
- `Arquitectura_Datos_Cache_Aside_Fundamental.pdf` × 3
- `Arquitectura_Datos_Consistencia_Fundamental.pdf` × 3
- `Arquitectura_Despliegue_Blue_Green_Fundamental.pdf` × 3
- `Arquitectura_Despliegue_Feature_Flags_Fundamental.pdf` × 3
- `Arquitectura_Seguridad_Arquitectura_Fundamental.pdf` × 3
- `Arquitectura_Micro_Service_Mesh_Fundamental.pdf` × 3
- `Arquitectura_Escalado_Horizontal_Fundamental.pdf` × 3
- `Arquitectura_Integracion_Messaging_Fundamental.pdf` × 3
- `Arquitectura_Integracion_Sync_Async_Fundamental.pdf` × 3
- `Arquitectura_Micro_Service_Discovery_Fundamental.pdf` × 3
- `Arquitectura_Escalado_Sharding_App_Fundamental.pdf` × 2
- `Arquitectura_Fiabilidad_Chaos_Fundamental.pdf` × 2
- `Arquitectura_Multi_Tenant_Fundamental.pdf` × 2

**`Bases_de_Datos`** (+9):
- `Bases_Datos_Topup_ACID_Isolation_Fundamental.pdf` × 1
- `Bases_Datos_Topup_Indexing_BTree_Fundamental.pdf` × 1
- `Bases_Datos_Topup_Indexing_Hash_Fundamental.pdf` × 1
- `Bases_Datos_Topup_Locks_Deadlock_Fundamental.pdf` × 1
- `Bases_Datos_Topup_NoSQL_LSM_Fundamental.pdf` × 1
- `Bases_Datos_Topup_Particion_Hash_Fundamental.pdf` × 1
- `Bases_Datos_Topup_Query_Planner_Fundamental.pdf` × 1
- `Bases_Datos_Topup_Replication_Sync_Fundamental.pdf` × 1
- `Bases_Datos_Topup_Transactions_2PC_Fundamental.pdf` × 1

**`Lenguajes_Programacion`** (+12):
- `Lenguajes_Topup_Java_Concurrent_Fundamental.pdf` × 2
- `Lenguajes_Topup_Java_GC_Fundamental.pdf` × 2
- `Lenguajes_Topup_Java_JMM_Fundamental.pdf` × 2
- `Lenguajes_Topup_Python_Asyncio_Fundamental.pdf` × 2
- `Lenguajes_Topup_Python_GIL_Fundamental.pdf` × 2
- `Lenguajes_Topup_Python_Typing_Fundamental.pdf` × 2

**`Sistemas_Operativos`** (+29):
- `Sistemas_Operativos_Topup_CFS_Completely_Fair_Fundamental.pdf` × 3
- `Sistemas_Operativos_Topup_CFS_Load_Balancing_Fundamental.pdf` × 3
- `Sistemas_Operativos_Topup_Mem_OOM_Fundamental.pdf` × 3
- `Sistemas_Operativos_Topup_Mem_Page_Cache_Fundamental.pdf` × 3
- `Sistemas_Operativos_Topup_Mem_Virtual_Paging_Fundamental.pdf` × 3
- `Sistemas_Operativos_Topup_Cgroups_v2_Fundamental.pdf` × 2
- `Sistemas_Operativos_Topup_IO_Block_Layer_Fundamental.pdf` × 2
- `Sistemas_Operativos_Topup_IO_Filesystem_Fundamental.pdf` × 2
- `Sistemas_Operativos_Topup_NS_PID_NET_Fundamental.pdf` × 2
- `Sistemas_Operativos_Topup_Scheduling_RT_Fundamental.pdf` × 2
- `Sistemas_Operativos_Topup_Signals_Process_Fundamental.pdf` × 2
- `Sistemas_Operativos_Topup_Sync_Futex_Fundamental.pdf` × 2

### D. Excluido del corpus (corte de raíz — **no** está en V5)

| Material | Motivo |
|----------|--------|
| `Agile.pdf` | Ceremonias / fuera de dominio técnico duro |
| `Manual de Estrategia y Control.pdf` | PESTEL / KPIs / liderazgo organizacional |
| `Ciberseg Cisco.pdf` | Ciberseguridad perimetral básica de usuario |
| Páginas con `.op` / «criterios operativos» | Molde residual de balanceo |
| Flags OCR basura (SHORT, TOC, CMD_LIST, …) | Calidad de extracción V1 |

> **Ciberseguridad** no es L1: el material AppSec residual se etiqueta como `Arquitectura` o `Redes_y_Comunicaciones`. El PDF Cisco **no** entra.

## Árbol completo (taxonomía de referencia)

```text
Tecnología
└── Computación e Informática
    ├── Hardware
    │   ├── Fundamentos de Hardware y Lógica Digital
    │   │   ├── 1. La Resistencia
    │   │   │   ├── BLOQUE 1 — Contexto Histórico
    │   │   │   │   ├── Origen (Georg Simon Ohm, 1827)
    │   │   │   │   ├── Fórmula fundamental: V = I · R
    │   │   │   │   └── Evolución tecnológica (bobinado → películas metálicas → implantación iónica)
    │   │   │   ├── BLOQUE 2 — Contexto Técnico
    │   │   │   │   ├── Definición y función de limitación de corriente
    │   │   │   │   └── Aplicaciones en computación digital (polarización, pull-up/pull-down, divisores)
    │   │   │   ├── BLOQUE 3 — Arquitectura del Concepto
    │   │   │   │   ├── Tipos de material (Carbono, NiCr, Cerámica)
    │   │   │   │   ├── Función de Ingeniería / Implicación Técnica
    │   │   │   │   └── Riesgos y Advertencias (ruido térmico, tolerancias, inductancia parásita)
    │   │   │   └── BLOQUE 4 — Ejemplo Progresivo
    │   │   ├── 2. El Capacitor
    │   │   │   ├── BLOQUE 1 — Contexto Histórico
    │   │   │   ├── BLOQUE 2 — Contexto Técnico
    │   │   │   │   ├── Almacenamiento en campos eléctricos
    │   │   │   │   └── Constantes de tiempo
    │   │   │   ├── BLOQUE 3 — Arquitectura del Concepto
    │   │   │   │   ├── Tipos (Cerámico BaTiO₃, Electrolítico de Aluminio, Tantalio, Low-k / High-k)
    │   │   │   │   ├── Función de Ingeniería / Implicación Técnica
    │   │   │   │   └── Riesgos y Advertencias
    │   │   │   │       ├── Efecto piezoeléctrico
    │   │   │   │       ├── Secado del electrolito
    │   │   │   │       ├── Ignición pirofórica (Tantalio)
    │   │   │   │       └── Corriente de fuga por tunelamiento cuántico
    │   │   │   └── BLOQUE 4 — Ejemplo Progresivo
    │   │   ├── 3. El Inductor
    │   │   │   ├── BLOQUE 1 — Contexto Histórico
    │   │   │   ├── BLOQUE 2 — Contexto Técnico (Ley de Faraday, campos magnéticos)
    │   │   │   ├── BLOQUE 3 — Arquitectura del Concepto
    │   │   │   └── BLOQUE 4 — Ejemplo Progresivo
    │   │   ├── 4. El Diodo
    │   │   │   ├── BLOQUE 1 — Contexto Histórico
    │   │   │   ├── BLOQUE 2 — Contexto Técnico (Unión P-N, Rectificación, Barreras de Potencial)
    │   │   │   ├── BLOQUE 3 — Arquitectura del Concepto
    │   │   │   └── BLOQUE 4 — Ejemplo Progresivo
    │   │   ├── 5. El Transistor (BJT y MOSFET)
    │   │   │   ├── BLOQUE 1 — Contexto Histórico
    │   │   │   ├── BLOQUE 2 — Contexto Técnico
    │   │   │   │   ├── Transistores de Unión Bipolar (BJT)
    │   │   │   │   └── Transistores de Efecto de Campo (MOSFET / CMOS)
    │   │   │   ├── BLOQUE 3 — Arquitectura del Concepto
    │   │   │   └── BLOQUE 4 — Ejemplo Progresivo
    │   │   ├── 6. Álgebra Booleana
    │   │   │   ├── BLOQUE 1 — Contexto Histórico / Teórico
    │   │   │   ├── BLOQUE 2 — Postulados Fundamentales y Teoremas de Conmutación
    │   │   │   ├── BLOQUE 3 — Compuertas Lógicas Discretas
    │   │   │   │   ├── AND, OR, NOT
    │   │   │   │   ├── NAND, NOR, XOR
    │   │   │   │   └── Simplificación de Funciones Lógicas (Reducción de Silicio)
    │   │   │   └── BLOQUE 4 — Ejemplo Progresivo
    │   │   ├── 7. Los Registros
    │   │   │   ├── BLOQUE 1 — Contexto
    │   │   │   ├── BLOQUE 2 — Almacenamiento Síncrono de Vectores de Bits y Flip-Flops
    │   │   │   ├── BLOQUE 3 — Arquitectura del Concepto
    │   │   │   └── BLOQUE 4 — Ejemplo Progresivo
    │   │   ├── 8. Los Contadores
    │   │   │   ├── BLOQUE 1 — Contexto
    │   │   │   ├── BLOQUE 2 — Circuitos Secuenciales para Temporización y Control de Ciclos de CPU
    │   │   │   ├── BLOQUE 3 — Arquitectura del Concepto
    │   │   │   └── BLOQUE 4 — Ejemplo Progresivo
    │   │   └── 9. Multiplexores y Demultiplexores
    │   │       ├── BLOQUE 1 — Contexto
    │   │       ├── BLOQUE 2 — Arquitecturas de Enrutamiento de Datos
    │   │       │   ├── Multiplexores (MUX) – Selección Vectorial y Llaves de Transmisión CMOS
    │   │       │   └── Demultiplexores (DEMUX) – Distribución de Señales
    │   │       ├── BLOQUE 3 — Fenómenos Parásitos (Capacitancia de Dispersión RC y Árboles Jerárquicos)
    │   │       └── BLOQUE 4 — Ejemplo Progresivo
    │   └── Arquitectura de Computadores y Subsistemas          [APLICADO v3 — 52 fragmentos]
    │   │   │
    │   │   ├── 1. Arquitectura de Computadores (modelo de sistema)
    │   │   │   ├── BLOQUE 1 — Contexto Histórico
    │   │   │   │   ├── Von Neumann vs Harvard
    │   │   │   │   ├── Evolución de buses y ciclos de instrucción
    │   │   │   │   └── Miniaturización y SoC
    │   │   │   ├── BLOQUE 2 — Contexto Técnico
    │   │   │   │   ├── Ciclo fetch–decode–execute
    │   │   │   │   ├── Datapath y control unit (visión de bloque)
    │   │   │   │   └── Interrupciones y excepciones (rol en el sistema)
    │   │   │   ├── BLOQUE 3 — Arquitectura del Concepto
    │   │   │   │   ├── RISC vs CISC (compromisos)
    │   │   │   │   ├── Pipelining y hazards (idea operativa)
    │   │   │   │   └── Multinúcleo y coherencia (introducción)
    │   │   │   └── BLOQUE 4 — Ejemplo Progresivo
    │   │   │       └── Lectura de un path de instrucción simple de punta a punta
    │   │   │
    │   │   ├── 2. Procesadores y CPU
    │   │   │   ├── BLOQUE 1 — Contexto Histórico
    │   │   │   │   ├── Generaciones de CPU
    │   │   │   │   └── Métricas clásicas (clock, IPC — a nivel conceptual)
    │   │   │   ├── BLOQUE 2 — Contexto Técnico
    │   │   │   │   ├── ALU, registros, PC, IR
    │   │   │   │   ├── Modos de direccionamiento (idea)
    │   │   │   │   └── SIMD / extensiones (cuándo importan)
    │   │   │   ├── BLOQUE 3 — Arquitectura del Concepto
    │   │   │   │   ├── Superescalar y out-of-order (visión)
    │   │   │   │   ├── SMT / hyper-threading (trade-off)
    │   │   │   │   └── Thermal Design Power y límites físicos
    │   │   │   └── BLOQUE 4 — Ejemplo Progresivo
    │   │   │       └── Del ensamblador mínimo al impacto en rendimiento percibido
    │   │   │
    │   │   ├── 3. Memoria (RAM, ROM, Caché)
    │   │   │   ├── BLOQUE 1 — Contexto Histórico
    │   │   │   │   ├── De núcleos de ferrita a DRAM/SRAM
    │   │   │   │   └── Pirámide de memoria como respuesta a la brecha CPU–memoria
    │   │   │   ├── BLOQUE 2 — Contexto Técnico
    │   │   │   │   ├── RAM volátil vs ROM / firmware
    │   │   │   │   ├── Localidad temporal y espacial
    │   │   │   │   └── Latencia vs ancho de banda
    │   │   │   ├── BLOQUE 3 — Arquitectura del Concepto
    │   │   │   │   ├── Caché L1/L2/L3 y líneas de caché
    │   │   │   │   ├── Hit / miss / miss penalty
    │   │   │   │   ├── Write-through vs write-back
    │   │   │   │   └── Memoria virtual (puente con SO; foco en hardware MMU)
    │   │   │   └── BLOQUE 4 — Ejemplo Progresivo
    │   │   │       └── Por qué un acceso “aleatorio” destruye el throughput
    │   │   │
    │   │   ├── 4. Almacenamiento (HDD, SSD, NVMe)
    │   │   │   ├── BLOQUE 1 — Contexto Histórico
    │   │   │   │   ├── Medios magnéticos → flash
    │   │   │   │   └── Interfaz: PATA/SATA → NVMe
    │   │   │   ├── BLOQUE 2 — Contexto Técnico
    │   │   │   │   ├── Latencia mecánica vs electrónica
    │   │   │   │   ├── IOPS y colas de comandos
    │   │   │   │   └── Endurance y wear leveling (SSD)
    │   │   │   ├── BLOQUE 3 — Arquitectura del Concepto
    │   │   │   │   ├── Jerarquía: registro → caché → RAM → SSD/HDD
    │   │   │   │   ├── Controllers y DMA (idea)
    │   │   │   │   └── NVMe sobre PCIe
    │   │   │   └── BLOQUE 4 — Ejemplo Progresivo
    │   │   │       └── Elegir medio según workload (secuencial vs aleatorio)
    │   │   │
    │   │   ├── 5. Placas Base, Buses y Chipset
    │   │   │   ├── BLOQUE 1 — Contexto Histórico
    │   │   │   ├── BLOQUE 2 — Contexto Técnico
    │   │   │   │   ├── Socket, chipset, slots de expansión
    │   │   │   │   └── PCIe lanes y norte/sur (evolución del chipset)
    │   │   │   ├── BLOQUE 3 — Arquitectura del Concepto
    │   │   │   │   ├── Bus de sistema y cuellos de botella
    │   │   │   │   ├── Firmware/UEFI (rol de arranque)
    │   │   │   │   └── Alimentación y VRM (implicación de estabilidad)
    │   │   │   └── BLOQUE 4 — Ejemplo Progresivo
    │   │   │
    │   │   └── 6. Periféricos y Dispositivos de E/S
    │   │       ├── BLOQUE 1 — Contexto Histórico
    │   │       ├── BLOQUE 2 — Contexto Técnico
    │   │       │   ├── Memory-mapped I/O vs port I/O (idea)
    │   │       │   ├── Interrupciones de dispositivo y polling
    │   │       │   └── USB / controladores de host (visión)
    │   │       ├── BLOQUE 3 — Arquitectura del Concepto
    │   │       │   ├── Pipeline de E/S: dispositivo → bus → driver (límite con SO)
    │   │       │   └── Buffering y latencia de interacción
    │   │       └── BLOQUE 4 — Ejemplo Progresivo
    │
    ├── Software
    │   ├── Sistemas Operativos
    │   │   ├── Administración de Sistemas
    │   │   │   ├── 1. Introducción a la Virtualización
    │   │   │   │   └── Conceptos de Hipervisores y Virtualización
    │   │   │   ├── 2. Workshop: Implementación de Ubuntu Desktop
    │   │   │   │   └── 2.1 Configuración de la VM y Asignación de Recursos
    │   │   │   ├── 3. Operativa en la Terminal (CLI)
    │   │   │   │   ├── 3.1 Sistemas de Ayuda Integrada
    │   │   │   │   │   ├── man
    │   │   │   │   │   └── info
    │   │   │   │   ├── 3.2 Gestión de Archivos y Directorios
    │   │   │   │   │   ├── 3.2.1 Identificación de Tipos de Archivos (ls)
    │   │   │   │   │   ├── 3.2.2 Eliminación de Recursos (rm)
    │   │   │   │   │   └── 3.2.3 Renombrado y Movimiento (mv)
    │   │   │   │   ├── 3.3 Duplicación y Selección Masiva
    │   │   │   │   │   ├── 3.3.1 Uso del Comodín Asterisco (*)
    │   │   │   │   │   ├── 3.3.2 Copiado Recursivo de Directorios (cp -r)
    │   │   │   │   │   └── Sintaxis ejemplo: cp -r CarpetaOrigen CarpetaDestino/
    │   │   │   │   ├── 3.4 Jerarquía de Rutas
    │   │   │   │   │   ├── Rutas Absolutas (comienzan desde /)
    │   │   │   │   │   │   └── Ejemplo: /home/usuario/Documentos
    │   │   │   │   │   └── Rutas Relativas (desde el directorio actual)
    │   │   │   │   │       └── Ejemplo: ./Terminal/notas.txt
    │   │   │   │   └── 3.5 Flujos de Salida y Persistencia (I/O Redirection)
    │   │   │   │       ├── Sobreescritura (>)
    │   │   │   │       └── Anexado (>>)
    │   │   │   ├── 4. Edición de Configuración con VI/Vim
    │   │   │   │   ├── 4.1 Modos de Operación
    │   │   │   │   │   ├── Modo Comando (Esc)
    │   │   │   │   │   └── Modo Inserción (i)
    │   │   │   │   ├── 4.2 Comandos de Manipulación de Texto
    │   │   │   │   │   ├── yy → Copia la línea actual (yank)
    │   │   │   │   │   ├── dd → Corta/elimina la línea actual (delete)
    │   │   │   │   │   ├── p → Pega el contenido
    │   │   │   │   │   └── A → Ir al final de línea + modo inserción
    │   │   │   │   ├── 4.3 Gestión de Archivos y Salida
    │   │   │   │   │   ├── :w [nombre] → Guardar con nombre
    │   │   │   │   │   ├── :w → Guardar
    │   │   │   │   │   ├── :q → Salir
    │   │   │   │   │   ├── :wq → Guardar y salir
    │   │   │   │   │   ├── :q! → Forzar salida descartando cambios
    │   │   │   │   │   └── :set number → Mostrar números de línea
    │   │   │   │   └── 4.4 Búsqueda y Sustitución
    │   │   │   ├── 5. Procesamiento y Análisis de Texto
    │   │   │   │   ├── 5.1 Métricas de Archivos (wc)
    │   │   │   │   ├── 5.2 Filtrado de Duplicados (uniq)
    │   │   │   │   └── 5.3 Ordenamiento de Datos (sort)
    │   │   │   ├── 6. Automatización y Redes
    │   │   │   │   ├── 6.1 Diagnóstico de Conexiones (ping, ifconfig / ip)
    │   │   │   │   ├── 6.2 Transferencia de Archivos (scp, rsync)
    │   │   │   │   └── 6.3 Creación de Scripts de Respaldo Automatizado (backup.sh)
    │   │   │   └── 7. Administración del Sistema (Sysadmin)
    │   │   │       ├── 7.1 Actualización del Sistema (apt update / upgrade)
    │   │   │       ├── 7.2 Instalación Práctica: Servidor Web (Apache) y Base de Datos (MySQL)
    │   │   │       ├── 7.3 Gestión de Servicios de Sistema (systemctl start / enable / disable)
    │   │   │       ├── 7.4 Estructura Estándar del Sistema de Archivos FHS
    │   │   │       │   └── /, /bin, /sbin, /etc, /var, /home, etc.
    │   │   │       └── 7.5 Gestión de Logs y Gobernanza de Auditoría
    │   │   │           ├── /var/log/syslog
    │   │   │           ├── auth.log
    │   │   │           ├── journalctl
    │   │   │           └── tail -f
    │   │   └── Linux y núcleo moderno                    [EN CORPUS V5 — lotes SO + top-up]
    │   │       ├── Planificación CFS
    │   │       │   ├── vruntime, red-black tree, nice/weight
    │   │       │   ├── Load balancing entre CPUs
    │   │       │   ├── Preempción, ticks y latencia de scheduling
    │   │       │   └── SCHED_FIFO / SCHED_RR (tiempo real)
    │   │       ├── Memoria virtual y page cache
    │   │       │   ├── Address spaces y aislamiento
    │   │       │   ├── Page tables multi-nivel y TLB
    │   │       │   ├── Page fault minor/major, demand paging, COW
    │   │       │   ├── Working set, thrashing y swap
    │   │       │   ├── Dirty pages, writeback y reclaim LRU
    │   │       │   └── OOM killer, oom_score_adj y cgroup memory.max
    │   │       ├── I/O y filesystems
    │   │       │   ├── Camino VFS → block layer → driver
    │   │       │   ├── bio, request queues, schedulers MQ
    │   │       │   ├── ext4 journaling, extents, fsync / O_DSYNC
    │   │       │   └── Buffered vs direct I/O
    │   │       ├── Namespaces y cgroups / contenedores
    │   │       │   ├── Namespaces: pid, net, mnt, user, UTS, IPC, cgroup
    │   │       │   ├── cgroups v1 vs v2 (cpu, memory, io)
    │   │       │   ├── PSI y adaptación de carga
    │   │       │   ├── Superficie de ataque del kernel compartido
    │   │       │   └── Runtimes (runc / containerd) y delegación
    │   │       └── Sincronización
    │   │           ├── Futex (wait/wake, contención)
    │   │           ├── Threads POSIX, kernel threads
    │   │           ├── Spinlock, mutex, RCU (visión SO)
    │   │           └── Señales POSIX (handlers, máscara, SIGCHLD)
    │   │
    │   ├── Lenguajes de Programación
    │   │   ├── Java
    │   │   │   ├── 1. Introducción al Lenguaje Java
    │   │   │   │   ├── 1.1 Objetivos de Aprendizaje
    │   │   │   │   ├── 1.2 Contexto y Evolución del Lenguaje
    │   │   │   │   ├── 1.3 El Proceso de Traducción: Compiled vs. Interpreted
    │   │   │   │   ├── 1.4 El Problema de Portability y la Solución de Java (JVM + Bytecode)
    │   │   │   │   └── 1.5 Escritura, Compilación y Ejecución del Código Java
    │   │   │   │       └── 1.5.1 Modo Interpretado de Java y JShell
    │   │   │   ├── 2. Variables y Tipos de Datos
    │   │   │   │   ├── 2.1 Representación de la Información: Variables
    │   │   │   │   ├── 2.2 Identificadores (Identifiers)
    │   │   │   │   ├── 2.3 El Concepto de Tipado Fuerte (Strongly Typed)
    │   │   │   │   ├── 2.4 Categorías de Tipos de Datos
    │   │   │   │   │   ├── 2.4.1 Tipos Primitivos (Primitive Types)
    │   │   │   │   │   └── 2.4.2 Tipos de Objeto (Object / Reference Types)
    │   │   │   │   ├── 2.5 Profundización: Valores Simples vs. Comportamientos
    │   │   │   │   ├── 2.6 Reglas y Convenciones de Identificadores
    │   │   │   │   ├── 2.7 Inicialización de Variables y Constantes
    │   │   │   │   │   └── 2.7.1 Constantes (final)
    │   │   │   │   └── 2.8 Inferencia de Tipos (var)
    │   │   │   ├── 3. Operadores y Expresiones
    │   │   │   │   ├── 3.1 Operadores de Asignación y Aritméticos
    │   │   │   │   ├── 3.2 Asignación Compuesta (Compound Assignment)
    │   │   │   │   ├── 3.3 Operadores de Incremento y Decremento
    │   │   │   │   │   └── 3.3.1 La Diferencia Crítica: Prefix vs. Postfix
    │   │   │   │   ├── 3.4 Operadores Relacionales y de Igualdad
    │   │   │   │   ├── 3.5 Operadores Lógicos (AND / OR)
    │   │   │   │   ├── 3.6 Operador Ternario (Conditional Assignment)
    │   │   │   │   ├── 3.7 Jerarquía y Precedencia de Operadores
    │   │   │   │   │   └── 3.7.1 Análisis de una Expresión Compleja
    │   │   │   │   ├── 3.8 Tipos de Resultado y Promoción Numérica
    │   │   │   │   └── 3.9 Conversión Explícita de Tipos (Typecasting)
    │   │   │   │       ├── 3.9.1 Ampliación vs. Reducción (Widening vs. Narrowing)
    │   │   │   │       └── 3.9.2 Truncamiento vs. Redondeo (Math.round)
    │   │   │   ├── 4. Manejo de Cadenas de Texto (Strings) y Caracteres
    │   │   │   │   ├── 4.1 Representación Básica y Delimitadores
    │   │   │   │   ├── 4.2 Caracteres de Escape (Escape Characters)
    │   │   │   │   ├── 4.3 Inicialización y Optimización de Memoria
    │   │   │   │   ├── 4.4 Inmutabilidad Estructural de String
    │   │   │   │   ├── 4.5 Métodos Operativos de la Clase String
    │   │   │   │   │   ├── 4.5.1 Evaluación de Longitud e Índices
    │   │   │   │   │   ├── 4.5.2 Búsqueda y Localización
    │   │   │   │   │   ├── 4.5.3 Extracción y Transformación
    │   │   │   │   │   └── 4.5.4 Concatenación
    │   │   │   │   └── 4.6 Clases Envolventes (Wrapper Classes)
    │   │   │   ├── 5. Estructuras de Datos: Arreglos (Arrays)
    │   │   │   │   ├── 5.1 Definición y Propiedades Estructurales
    │   │   │   │   ├── 5.2 Declaración e Inicialización
    │   │   │   │   ├── 5.3 Valores por Defecto
    │   │   │   │   ├── 5.4 Acceso, Asignación e Inicialización en Línea
    │   │   │   │   └── 5.5 Errores Comunes y Prevención
    │   │   │   ├── 6. Estructuras de Control de Flujo
    │   │   │   │   ├── 6.1 Bifurcaciones Condicionales: if/else
    │   │   │   │   │   ├── 6.1.1 Reglas de Sintaxis e Indentación
    │   │   │   │   │   ├── 6.1.2 Anidamiento y la Ausencia de elseif
    │   │   │   │   │   └── 6.1.3 if/else vs. Operador Ternario
    │   │   │   │   ├── 6.2 Ejecución Iterativa (Loops)
    │   │   │   │   │   ├── 6.2.1 Bucle while y do/while
    │   │   │   │   │   ├── 6.2.2 Bucle for
    │   │   │   │   │   ├── 6.2.3 Bucle for-each (Enhanced For Loop)
    │   │   │   │   │   ├── 6.2.4 Interrupción de Flujo: continue y break
    │   │   │   │   │   └── 6.2.5 Anidamiento de Bucles
    │   │   │   │   └── 6.3 Selección Múltiple y Saltos de Flujo: switch/case
    │   │   │   │       ├── 6.3.1 Reglas y Tipos Soportados
    │   │   │   │       ├── 6.3.2 Agrupación de Casos y el Operador break
    │   │   │   │       ├── 6.3.3 Sintaxis de Flecha (Arrow Syntax)
    │   │   │   │       └── 6.3.4 Expresiones switch (Switch Expressions)
    │   │   │   ├── 7. Modularización y Métodos (Methods)
    │   │   │   │   ├── 7.1 Concepto y Propósito
    │   │   │   │   ├── 7.2 Anatomía y Definición de un Método
    │   │   │   │   ├── 7.3 Convenciones de Nomenclatura
    │   │   │   │   └── 7.4 Variables Locales y Alcance Léxico (Scope)
    │   │   │   │       ├── 7.4.1 Regla de Inicialización
    │   │   │   │       └── 7.4.2 El Concepto de Scope
    │   │   │   ├── 8. Fundamentos de Orientación a Objetos: Clases y Objetos
    │   │   │   │   ├── 8.1 El Concepto de Abstracción
    │   │   │   │   ├── 8.2 Estructura y Definición de una Clase
    │   │   │   │   ├── 8.3 Diferenciación entre Clase e Instancia (Objeto)
    │   │   │   │   ├── 8.4 Instanciación y Asignación de Memoria: El Operador new
    │   │   │   │   ├── 8.5 Constructores (Constructors)
    │   │   │   │   ├── 8.6 Gestión de Referencias y Acceso a Objetos
    │   │   │   │   └── 8.7 Ausencia de Referencia: null
    │   │   │   ├── 9. Propiedades de Objetos, Shadowing y la Referencia this
    │   │   │   │   ├── 9.1 Variables de Instancia vs. Variables Locales
    │   │   │   │   ├── 9.2 Métodos de Acceso (Getters y Setters) y Lógica de Validación
    │   │   │   │   ├── 9.3 Ocultamiento de Variables (Variable Shadowing)
    │   │   │   │   ├── 9.4 La Referencia Autodirigida this
    │   │   │   │   └── 9.5 Sobrecarga de Métodos (Method Overloading)
    │   │   │   │       ├── 9.5.1 Reglas de Resolución de Sobrecarga
    │   │   │   │       └── 9.5.2 Errores Comunes y Factores Irrelevantes
    │   │   │   ├── 10. Encapsulamiento Formal y Modificadores de Acceso
    │   │   │   │   ├── 10.1 El Principio de Encapsulamiento
    │   │   │   │   ├── 10.2 Modificadores de Acceso: public y private
    │   │   │   │   └── 10.3 Acceso por Defecto (Package-Private)
    │   │   │   ├── 11. Constructores y Reutilización de Lógica
    │   │   │   │   ├── 11.1 El Constructor por Defecto (No-arg Constructor)
    │   │   │   │   ├── 11.2 Constructores Personalizados y Sobrecarga
    │   │   │   │   └── 11.3 Encadenamiento de Constructores (Constructor Chaining)
    │   │   │   │       └── 11.3.1 Reglas Obligatorias del Encadenamiento
    │   │   │   ├── 12. Contexto de Clase y Miembros Estáticos (static)
    │   │   │   │   ├── 12.1 Diferenciación de Contextos: Instancia vs. Clase
    │   │   │   │   ├── 12.2 Variables de Clase (static)
    │   │   │   │   ├── 12.3 Métodos Estáticos (static)
    │   │   │   │   │   └── 12.3.1 Reglas de Acceso y Ambigüedad
    │   │   │   │   ├── 12.4 Constantes Globales (static final)
    │   │   │   │   └── 12.5 Ejemplos Nativos en el JDK
    │   │   │   ├── 13. Herencia (Inheritance)
    │   │   │   │   ├── 13.1 El Principio de Herencia
    │   │   │   │   ├── 13.2 Method Overriding y la anotación @Override
    │   │   │   │   ├── 13.3 La Referencia super
    │   │   │   │   ├── 13.4 Resolución Dinámica de Métodos
    │   │   │   │   ├── 13.5 La Clase Raíz Object
    │   │   │   │   ├── 13.6 Comparación de Objetos y el método equals()
    │   │   │   │   └── 13.7 Prevención de NullPointerException
    │   │   │   ├── 14. Manejo de Excepciones y Tolerancia a Fallos
    │   │   │   │   ├── 14.1 Concepto y Flujo de Interrupción
    │   │   │   │   ├── 14.2 Jerarquía y Clasificación de Excepciones
    │   │   │   │   │   ├── 14.2.1 Excepciones No Comprobadas (Unchecked)
    │   │   │   │   │   └── 14.2.2 Excepciones Comprobadas (Checked)
    │   │   │   │   ├── 14.3 Traza de Pila (Stack Trace)
    │   │   │   │   ├── 14.4 Bloques de Captura y Manejo: try / catch
    │   │   │   │   ├── 14.5 Generación y Propagación (throw y throws)
    │   │   │   │   ├── 14.6 Creación de Excepciones Personalizadas
    │   │   │   │   └── 14.7 Alteración del Flujo de Recuperación
    │   │   │   └── 15. Complemento: Paquetes, Módulos y Firma de Entrada
    │   │   │       ├── 15.1 Firma Formal del Método main
    │   │   │       ├── 15.2 Valores por Defecto en Variables de Instancia
    │   │   │       ├── 15.3 Organización mediante Paquetes
    │   │   │       ├── 15.4 Noción de Módulos
    │   │   │       └── 15.5 Capturas Múltiples y Propagación
    │   │   └── Python
    │   │       ├── Fundamentos del Lenguaje
    │   │       │   ├── 1. Fundamentos del Lenguaje Python
    │   │       │   │   ├── Filosofía de Diseño
    │   │       │   │   │   ├── Lenguaje interpretado de alto nivel
    │   │       │   │   │   ├── Sintaxis cercana al lenguaje natural
    │   │       │   │   │   └── Ejecución línea por línea (sin compilación previa)
    │   │       │   │   └── Áreas de Aplicación
    │   │       │   │       ├── Desarrollo Web
    │   │       │   │       ├── Análisis de Datos
    │   │       │   │       ├── Inteligencia Artificial
    │   │       │   │       └── Automatización
    │   │       │   ├── 2. Sintaxis y Control Estructural
    │   │       │   │   ├── 2.1 Sintaxis Python
    │   │       │   │   │   ├── Asignación con =
    │   │       │   │   │   ├── Asignaciones múltiples (a, b, c = 4, 3, 2)
    │   │       │   │   │   └── Ejemplo de código básico con print()
    │   │       │   │   ├── 2.2 Indentación y Bloques de Código
    │   │       │   │   │   ├── Representación de bloques mediante indentación
    │   │       │   │   │   ├── Norma: 4 espacios
    │   │       │   │   │   └── Ejemplo: if True: print("True")
    │   │       │   │   ├── 2.3 Función print()
    │   │       │   │   │   ├── Uso para depuración y visualización
    │   │       │   │   │   └── Sintaxis: print("texto") / print(variable)
    │   │       │   │   ├── 2.4 Variables y Tipos Básicos
    │   │       │   │   │   ├── Cadenas de texto (str)
    │   │       │   │   │   ├── Booleanos (True / False)
    │   │       │   │   │   └── Asignaciones
    │   │       │   │   └── 2.5 Operadores
    │   │       │   │       ├── Operadores de Asignación (=, +=, -=, *=, /=, %=)
    │   │       │   │       ├── Operadores Aritméticos (+, -, *, /, %, **, //)
    │   │       │   │       ├── Operadores Relacionales (==, !=, >, <, >=, <=)
    │   │       │   │       └── Operadores Lógicos (and, or, not)
    │   │       │   ├── 3. Estructuras Condicionales
    │   │       │   │   ├── Sentencia if básica
    │   │       │   │   ├── Combinación de condiciones (and / or)
    │   │       │   │   ├── Cláusula else
    │   │       │   │   └── Cláusula elif
    │   │       │   └── 4. Principios de Modularidad (Funciones)
    │   │       │       ├── Definición de funciones con def
    │   │       │       ├── Componentes de una función (Nombre, Código interno, Valor de retorno)
    │   │       │       └── Argumentos / Parámetros
    │   │       │           ├── Por posición
    │   │       │           ├── Por nombre
    │   │       │           ├── Por defecto
    │   │       │           └── De longitud variable
    │   │       └── Ecosistema de Interfaces (Tkinter)
    │   │           ├── 1. Introducción General a Tkinter
    │   │           │   ├── 1.1 Definición Breve
    │   │           │   ├── 1.2 Sintaxis Pura
    │   │           │   ├── 1.3 Desglose Sintáctico
    │   │           │   ├── 1.4 Ejemplo Mínimo
    │   │           │   └── 1.5 Nota Técnica de Arquitectura y Compatibilidad
    │   │           ├── 2. Arquitectura Interna de Tcl/Tk y Tkinter
    │   │           │   ├── 2.1 Definición Breve
    │   │           │   ├── 2.2 Sintaxis Pura
    │   │           │   ├── 2.3 Desglose Sintáctico
    │   │           │   └── 2.4 Ejemplo Mínimo
    │   │           ├── 3. Módulos Fundamentales de Tkinter
    │   │           ├── 4. Clase Tk (Ventana Raíz)
    │   │           ├── Widgets Clásicos
    │   │           │   ├── Widget Button
    │   │           │   ├── Widget Label
    │   │           │   ├── Widget Entry
    │   │           │   ├── Widget Text
    │   │           │   ├── Widget Frame
    │   │           │   ├── Widget Canvas
    │   │           │   ├── Widget Checkbutton
    │   │           │   ├── Widget Radiobutton
    │   │           │   ├── Widget Scrollbar
    │   │           │   ├── Widget Scale
    │   │           │   ├── Widget Spinbox
    │   │           │   ├── Widget Menu
    │   │           │   ├── Widget Menubutton
    │   │           │   ├── Widget PanedWindow
    │   │           │   ├── Widget LabelFrame
    │   │           │   └── Widget Toplevel
    │   │           ├── Diálogos y Componentes Adicionales
    │   │           │   ├── Diálogo Estándar colorchooser
    │   │           │   └── Diálogo Estándar OptionMenu
    │   │           ├── Widgets Modernos (ttk)
    │   │           │   └── Widget ttk.Button (y familia ttk)
    │   │           ├── Configuración y Estado
    │   │           │   ├── Opciones de Configuración de Widgets
    │   │           │   └── Asociación de Variables (StringVar, IntVar, BooleanVar, DoubleVar)
    │   │           ├── Sistema de Eventos
    │   │           │   ├── Eventos y Enlaces (bind)
    │   │           │   ├── Evento Avanzado bind_all
    │   │           │   ├── Evento Avanzado bind_class
    │   │           │   ├── Evento Avanzado event_add
    │   │           │   └── Evento Avanzado event_generate
    │   │           ├── Temporizadores y Concurrencia
    │   │           │   ├── Modelo de Subprocesos (Threading)
    │   │           │   ├── Temporizador Avanzado after_idle
    │   │           │   └── Temporizador Avanzado after_cancel
    │   │           ├── Gestión de Ventanas y Sistema
    │   │           │   ├── Gestor de Ventanas (wm)
    │   │           │   ├── Gestor de Archivos (createfilehandler)
    │   │           │   └── Índices (Entry, Text, Menu)
    │   │           └── Guía de Supervivencia de Tkinter
    │   │               └── Arquitectura Modular Definitiva Orientada a Objetos
    │   │                   ├── Herencia de Tk (Clase MasterApp)
    │   │                   ├── Separación de Lógica (Prevención de Tight Coupling)
    │   │                   ├── Uso exclusivo de Ttk (Coherencia Visual con el SO)
    │   │                   └── Ejemplo Mínimo Completo (Formulario asíncrono)
    │   │
    │   ├── Frameworks y Herramientas de Desarrollo
    │   │   └── Backend (FastAPI, Spring Boot)
    │   │
    │   ├── Bases de Datos y Persistencia
    │   │   └── Relacionales (SQL): MySQL, PostgreSQL, Oracle
    │   │       ├── Introducción y Alcance Operativo
    │   │       ├── 1. Configurando el ambiente y conociendo SQL
    │   │       │   ├── 1.1 Evolución Histórica y el Paradigma Relacional
    │   │       │   ├── 1.2 Estandarización ANSI y Taxonomía del Lenguaje
    │   │       │   │   ├── DDL (Data Definition Language)
    │   │       │   │   ├── DML (Data Manipulation Language)
    │   │       │   │   ├── DCL (Data Control Language)
    │   │       │   │   └── TCL (Transaction Control Language)
    │   │       │   ├── 1.3 Ventajas Operativas del Cumplimiento Estándar
    │   │       │   │   ├── Portabilidad
    │   │       │   │   ├── Interoperabilidad de Extracción (ETL)
    │   │       │   │   └── Desacoplamiento de Hardware
    │   │       │   ├── 1.4 Arquitectura y Evolución de MySQL
    │   │       │   │   ├── Origen (Axmark, Larsson, Widenius)
    │   │       │   │   ├── Diseño Open Source y Multithreaded
    │   │       │   │   └── Evolución tras adquisición por Sun Microsystems
    │   │       │   └── 1.5 Exploración Topológica mediante Entornos de Desarrollo Integrado (IDE)
    │   │       ├── 2. Filtrando las consultas de los datos
    │   │       │   ├── 2.1 Enrutamiento Espacial y Proyección Vectorial (SELECT, FROM)
    │   │       │   ├── 2.2 Mutación Semántica mediante Alias (AS)
    │   │       │   ├── 2.3 Filtrado Heurístico Horizontal (WHERE)
    │   │       │   ├── 2.4 Evaluación Matemática Continua y Mitigación de Coma Flotante
    │   │       │   ├── 2.5 Álgebra Booleana y Operadores Lógicos Condicionales (AND, OR, NOT)
    │   │       │   ├── 2.6 Instrumentación Práctica de Operadores Lógicos
    │   │       │   ├── 2.7 Agrupación Topológica y el Operador de Inclusión (IN)
    │   │       │   ├── 2.8 Coincidencia de Patrones y Búsqueda Heurística (LIKE)
    │   │       │   └── 2.9 Supresión de Duplicidad Vectorial y Proyección Única (DISTINCT)
    │   │       ├── 3. Presentación de los datos de una consulta
    │   │       │   ├── 3.1 Restricción de Cardinalidad de Salida (LIMIT)
    │   │       │   ├── 3.2 Ordenamiento Topológico y Clasificación Vectorial (ORDER BY)
    │   │       │   ├── 3.3 Agrupación Topológica y Funciones de Agregación (GROUP BY)
    │   │       │   └── 3.4 Mutación Lógica del Flujo de Datos (CASE)
    │   │       └── Advertencia Metodológica
    │   │           └── Fricción Arquitectónica y el Paradigma NoSQL
    │   │               ├── Limitaciones del modelo relacional ante Big Data
    │   │               └── Surgimiento de infraestructuras NoSQL
    │   │
    │   ├── Redes y Comunicaciones
    │   │   ├── Conceptos Básicos de Redes — Simulación y Evaluación en Cisco Packet Tracer
    │   │   │   ├── 1. Entorno de Simulación Orientado a Objetos (Packet Tracer)
    │   │   │   │   ├── 1.1 El Tipo de Archivo .pka (Packet Tracer Activity)
    │   │   │   │   │   ├── Definición y Propósitos Curriculares
    │   │   │   │   │   ├── Componentes Internos
    │   │   │   │   │   │   └── Ventana de Instrucciones
    │   │   │   │   │   └── Topología de Red Dual
    │   │   │   │   │       ├── Red Inicial (visible para el estudiante)
    │   │   │   │   │       └── Red de Respuesta (oculta – solución del instructor)
    │   │   │   │   └── 1.2 El Tipo de Archivo .pksz (Packet Tracer Assisted Activity)
    │   │   │   │       ├── Componentes Multimedia Integrados
    │   │   │   │       └── Motor de Secuencias de Comandos para Sugerencias Dinámicas
    │   │   │   └── 2. Pipeline de Evaluación Automatizada y Auditoría
    │   │   │       ├── 2.1 Mecanismos de Calificación en Tiempo de Ejecución (Runtime Scoring)
    │   │   │       ├── 2.2 Verificación de Objetivos mediante Check Results
    │   │   │       └── 2.3 Gestión de Logs e Historial de Modificaciones del Alumno
    │   │   ├── Guía Técnica: Fundamentos de Redes y Protocolos Web
    │   │   │   ├── 1. Contexto Histórico: El Proyecto World Wide Web
    │   │   │   │   └── Origen Institucional en el CERN y Evolución del Hipertexto
    │   │   │   ├── 2. Fundamentos de Redes y Protocolos de Transporte
    │   │   │   │   ├── 2.1 Introducción a las Redes y el Modelo TCP/IP
    │   │   │   │   │   ├── 2.1.1 Evolución Histórica y Estandarización: ARPANET y RFCs
    │   │   │   │   │   ├── 2.1.2 Arquitectura del Modelo TCP/IP (Referencia Estructural)
    │   │   │   │   │   ├── 2.1.3 Tipologías de Red (LAN, WAN, Topologías Lógicas)
    │   │   │   │   │   └── 2.1.4 Inspección Práctica Adicional del Modelo TCP/IP
    │   │   │   │   │       ├── traceroute
    │   │   │   │   │       ├── ss / netstat
    │   │   │   │   │       ├── tcpdump
    │   │   │   │   │       ├── curl
    │   │   │   │   │       └── nmap
    │   │   │   │   ├── 2.2 Protocolos de Transporte: TCP vs UDP
    │   │   │   │   │   ├── 2.2.1 Arquitectura Comparativa y Desglose Técnico
    │   │   │   │   │   │   ├── TCP (RFC 793) — Orientado a conexión, confiable y ordenado
    │   │   │   │   │   │   └── UDP (RFC 768) — Sin conexión, baja latencia
    │   │   │   │   │   └── Multiplexación mediante puertos
    │   │   │   │   ├── 2.3 Direccionamiento IP, Puertos y Sockets
    │   │   │   │   │   └── 2.3.1 Arquitectura de Direccionamiento y Multiplexación Lógica
    │   │   │   │   ├── 2.4 Sistema de Nombres de Dominio (DNS)
    │   │   │   │   │   ├── 2.4.1 Mecanismo de Resolución (Inversa y Recursiva)
    │   │   │   │   │   ├── 2.4.2 Caché DNS y Optimización de Latencia
    │   │   │   │   │   └── 2.4.3 Topología de Dominios de Nivel Superior (TLDs)
    │   │   │   │   └── 2.5 Métricas de Rendimiento: Latencia y Confiabilidad
    │   │   │   │       └── 2.5.1 Desglose Técnico y Factores de Influencia
    │   │   │   ├── 3. Arquitectura Cliente-Servidor y Protocolo HTTP
    │   │   │   │   └── 3.1 Métodos Operativos y Desglose Técnico
    │   │   │   ├── 4. Seguridad en HTTP/HTTPS e Infraestructura de Claves
    │   │   │   │   ├── 4.1 Protocolo de Enlace y Auditoría Criptográfica
    │   │   │   │   └── TLS 1.3 y AES-256-GCM
    │   │   │   └── 5. Desarrollo Web, APIs y Cloud Computing
    │   │   │       └── 5.1 Herramientas de Depuración (DevTools)
    │   │   │           ├── Panel Network → Ciclo Petición-Respuesta e Inspección de Cabeceras
    │   │   │           ├── Panel Performance → Renderizado y Consumo de CPU
    │   │   │           └── Panel Security → Validación Criptográfica
    │   │   ├── Modelos OSI y TCP/IP
    │   │   ├── Protocolos (HTTP, HTTPS, REST, GraphQL, gRPC, WebSockets)
    │   │   └── Diseño de APIs
    │   │
    │   ├── Inteligencia Artificial, Machine Learning y Ciencia de Datos
    │   │   ├── Inteligencia Artificial y sistemas de texto                 [APLICADO v5 — 46 fragmentos]
    │   │   │   │
    │   │   │   ├── 1. Clasificación de texto técnico y taxonomías
    │   │   │   │   ├── BLOQUE 1 — Contexto
    │   │   │   │   │   ├── Organización de documentación técnica por categorías
    │   │   │   │   │   └── Etiquetas de primer nivel (taxonomía controlada)
    │   │   │   │   ├── BLOQUE 2 — Contexto Técnico
    │   │   │   │   │   ├── Tokenización, n-gramas, TF-IDF
    │   │   │   │   │   ├── Clasificador lineal (p. ej. Logistic Regression) + TF-IDF
    │   │   │   │   │   └── Embeddings + cabeza de clasificación (visión)
    │   │   │   │   ├── BLOQUE 3 — Arquitectura del Concepto
    │   │   │   │   │   ├── ground truth vs predicción; train / validation / test
    │   │   │   │   │   ├── Multiclase vs multilabel
    │   │   │   │   │   └── Una etiqueta dominante por fragmento (diseño habitual en MVP de catálogo)
    │   │   │   │   └── BLOQUE 4 — Ejemplo Progresivo
    │   │   │   │       └── Del texto crudo al vector TF-IDF y a la clase predicha
    │   │   │   │
    │   │   │   ├── 2. Entrenamiento, métricas y desbalance
    │   │   │   │   ├── BLOQUE 1 — Contexto
    │   │   │   │   │   ├── Accuracy engañosa con clases desbalanceadas
    │   │   │   │   │   └── Ratio entre clase mayor y menor en un corpus real
    │   │   │   │   ├── BLOQUE 2 — Contexto Técnico
    │   │   │   │   │   ├── Precision, Recall, F1 (por clase y macro)
    │   │   │   │   │   ├── Matriz de confusión entre clases semánticamente cercanas
    │   │   │   │   │   └── class_weight, oversampling, aumento de datos
    │   │   │   │   ├── BLOQUE 3 — Arquitectura del Concepto
    │   │   │   │   │   ├── Data leakage
    │   │   │   │   │   ├── Validación cruzada estratificada (idea)
    │   │   │   │   │   └── Re-etiquetado vs ampliación de corpus
    │   │   │   │   └── BLOQUE 4 — Ejemplo Progresivo
    │   │   │   │       └── Interpretar F1 bajo en clase minoritaria y plan de remedio
    │   │   │   │
    │   │   │   ├── 3. Pipelines de fragmentos y quality gates
    │   │   │   │   ├── BLOQUE 1 — Contexto
    │   │   │   │   │   ├── Extracción desde documentos a filas
    │   │   │   │   │   └── Lineage: origen, página, marca de tiempo
    │   │   │   │   ├── BLOQUE 2 — Contexto Técnico
    │   │   │   │   │   ├── Schema tabular de fragmentos (id, texto, etiqueta, longitud, hash)
    │   │   │   │   │   ├── hash criptográfico y deduplicación
    │   │   │   │   │   └── Umbral de longitud (p. ej. percentil + mínimo absoluto)
    │   │   │   │   ├── BLOQUE 3 — Arquitectura del Concepto
    │   │   │   │   │   ├── Corpus general vs subconjunto de entrenamiento
    │   │   │   │   │   ├── Identificadores sintéticos vs extraídos
    │   │   │   │   │   └── Lotes parciales → fusión → recorte de train
    │   │   │   │   └── BLOQUE 4 — Ejemplo Progresivo
    │   │   │   │       └── Por qué un fragmento corto queda fuera del train
    │   │   │   │
    │   │   │   ├── 4. RAG, embeddings y chunking
    │   │   │   │   ├── BLOQUE 1 — Contexto
    │   │   │   │   │   ├── Clasificar un texto vs recuperar pasajes relevantes
    │   │   │   │   │   └── RAG: retrieval + generación (roles distintos)
    │   │   │   │   ├── BLOQUE 2 — Contexto Técnico
    │   │   │   │   │   ├── Embeddings; similitud coseno
    │   │   │   │   │   ├── Chunking y solape
    │   │   │   │   │   └── Filtros por metadata (categoría, documento)
    │   │   │   │   ├── BLOQUE 3 — Arquitectura del Concepto
    │   │   │   │   │   ├── Índice vectorial (concepto)
    │   │   │   │   │   ├── Top-k y re-ranking (idea)
    │   │   │   │   │   └── Desambiguación de términos homónimos en retrieval
    │   │   │   │   └── BLOQUE 4 — Ejemplo Progresivo
    │   │   │   │       └── Consulta en lenguaje natural → top-k fragmentos
    │   │   │   │
    │   │   │   ├── 5. API de clasificación y contratos
    │   │   │   │   ├── BLOQUE 1 — Contexto
    │   │   │   │   │   ├── Exponer un modelo por HTTP
    │   │   │   │   │   └── Clientes: aplicaciones y scripts
    │   │   │   │   ├── BLOQUE 2 — Contexto Técnico
    │   │   │   │   │   ├── Request de texto; response con clase, score, keywords (patrón habitual)
    │   │   │   │   │   ├── Validación de entrada y códigos 4xx / 5xx
    │   │   │   │   │   └── Timeouts y límites de tamaño de payload
    │   │   │   │   ├── BLOQUE 3 — Arquitectura del Concepto
    │   │   │   │   │   ├── Entrenamiento offline vs inferencia online
    │   │   │   │   │   ├── Versionado de modelo en la respuesta
    │   │   │   │   │   └── OpenAPI / schema de contrato
    │   │   │   │   └── BLOQUE 4 — Ejemplo Progresivo
    │   │   │   │       └── Del JSON de entrada a la etiqueta y la confianza
    │   │   │   │
    │   │   │   └── 6. Ciclo de vida del modelo y despliegue
    │   │   │   ├── BLOQUE 1 — Contexto
    │   │   │   │   ├── Artefacto entrenado vs código del servicio
    │   │   │   │   └── Reproducibilidad mínima (versiones de librerías — idea)
    │   │   │   ├── BLOQUE 2 — Contexto Técnico
    │   │   │   │   ├── Serialización de pipelines (vectorizer + clasificador)
    │   │   │   │   ├── Almacenamiento de artefactos (object storage conceptual)
    │   │   │   │   └── Carga del modelo al iniciar el servicio
    │   │   │   ├── BLOQUE 3 — Arquitectura del Concepto
    │   │   │   │   ├── Ambientes y promoción de modelos
    │   │   │   │   ├── Latencia de inferencia (idea de monitoreo)
    │   │   │   │   └── Reentrenamiento cuando cambia el corpus
    │   │   │   └── BLOQUE 4 — Ejemplo Progresivo
    │   │   │       └── Nuevo lote de datos → retrain → nuevo artefacto → servicio actualizado
    │   │   ├── Fundamentos de ML
    │   │   │   ├── 1. Introduction
    │   │   │   │   ├── Transformación industrial mediante AI
    │   │   │   │   │   ├── Manufactura
    │   │   │   │   │   ├── Atención médica
    │   │   │   │   │   ├── Educación
    │   │   │   │   │   ├── Comercio minorista (Retail)
    │   │   │   │   │   ├── Ciencias de la vida
    │   │   │   │   │   ├── Transporte
    │   │   │   │   │   └── Medios y entretenimiento
    │   │   │   │   └── Mecanismos de transformación
    │   │   │   │       ├── Text Summarization
    │   │   │   │       ├── Generación de código, imágenes, video y texto
    │   │   │   │       ├── Creación musical y localización de contenido
    │   │   │   │       ├── Chatbots y asistencia virtual
    │   │   │   │       ├── Detección de anomalías
    │   │   │   │       └── Analíticas de centros de contacto
    │   │   │   ├── 1.1 Definiciones y Jerarquía Conceptual
    │   │   │   │   ├── Inteligencia Artificial (AI)
    │   │   │   │   │   └── Campo amplio: percepción, razonamiento, aprendizaje, toma de decisiones
    │   │   │   │   ├── Machine Learning (ML)
    │   │   │   │   │   └── Subconjunto de AI basado en aprendizaje a partir de datos
    │   │   │   │   ├── Deep Learning (DL)
    │   │   │   │   │   └── Subconjunto de ML basado en redes neuronales multicapa
    │   │   │   │   └── Inteligencia Artificial Generativa (Generative AI)
    │   │   │   │       └── Subconjunto avanzado de DL + Foundation Models (FMs)
    │   │   │   ├── 2. Técnicas y Paradigmas de Machine Learning
    │   │   │   │   ├── 2.1 Fundamentos y Aplicabilidad del Machine Learning
    │   │   │   │   ├── 2.2 Técnicas y Paradigmas
    │   │   │   │   │   ├── 2.2.1 Aprendizaje Supervisado
    │   │   │   │   │   │   └── Casos de uso: detección de fraudes, clasificación de imágenes, retención
    │   │   │   │   │   └── 2.2.2 Aprendizaje No Supervisado
    │   │   │   │   │       └── Casos de uso: segmentación de clientes, marketing dirigido, visualización
    │   │   │   │   ├── 2.3 Capacidades de la Inteligencia Artificial Generativa
    │   │   │   │   │   ├── Adaptabilidad
    │   │   │   │   │   ├── Capacidad de respuesta (Responsiveness)
    │   │   │   │   │   ├── Simplificación de procesos
    │   │   │   │   │   ├── Creatividad y exploración
    │   │   │   │   │   ├── Eficiencia de datos
    │   │   │   │   │   └── Personalización
    │   │   │   │   └── 2.4 Criterios de Selección para Modelos Generativos
    │   │   │   ├── 3. Casos de Uso y Aplicaciones Industriales
    │   │   │   │   ├── 3.1.1 Medios y Entretenimiento
    │   │   │   │   ├── 3.1.2 Comercio Minorista (Retail)
    │   │   │   │   ├── 3.1.3 Atención Médica
    │   │   │   │   ├── 3.1.4 Ciencias de la Vida
    │   │   │   │   ├── 3.1.5 Servicios Financieros
    │   │   │   │   ├── 3.1.6 Manufactura
    │   │   │   │   └── 3.2 Categorías de Aplicaciones y Valor Comercial
    │   │   │   │       ├── Visión por Computadora (Computer Vision)
    │   │   │   │       ├── Procesamiento de Lenguaje Natural (NLP)
    │   │   │   │       └── Detección de Fraudes
    │   │   │   └── 4. Desafíos y Limitaciones de la Inteligencia Artificial Generativa
    │   │   │       ├── Riesgos sociales
    │   │   │       ├── Alucinaciones (Hallucinations)
    │   │   │       ├── Interpretabilidad (Modelos de Caja Negra)
    │   │   │       └── Control del No Determinismo y Sesgos (Bias)
    │   │   ├── NLP / Procesamiento de Texto
    │   │   │   ├── Retrieval-Augmented Generation (RAG)
    │   │   │   │   ├── Fundamentos y Contexto Empresarial
    │   │   │   │   ├── De Datasets Empresariales a Embeddings Vectoriales
    │   │   │   │   │   ├── Proyección Topológica (Embeddings)
    │   │   │   │   │   └── Similitud semántica por proximidad vectorial
    │   │   │   │   ├── Almacenamiento y Recuperación Vectorial (Vector Databases)
    │   │   │   │   │   ├── Algoritmos de similitud (k-Nearest Neighbors, Cosine Similarity)
    │   │   │   │   │   └── Ecosistema AWS (Amazon OpenSearch Service)
    │   │   │   │   └── Integración de RAG en arquitecturas empresariales
    │   │   │   └── Agentes Autónomos (Agentic AI)
    │   │   │       ├── Fundamentos de la Orquestación mediante Agentes
    │   │   │       │   ├── ReAct
    │   │   │       │   ├── Chain-of-Thought
    │   │   │       │   └── Funciones Críticas de los Agentes
    │   │   │       └── Implementación Arquitectónica Multi-Agente
    │   │   │           ├── Agente Ejecutor Transaccional
    │   │   │           ├── Agente de Ingesta Continua
    │   │   │           └── Agente Supervisor de Telemetría
    │   │   ├── MLOps
    │   │   │   ├── Evaluación de Rendimiento de Modelos Fundacionales
    │   │   │   │   ├── Fundamentos de la Evaluación Algorítmica
    │   │   │   │   ├── Construcción de un Benchmark Dataset (Ground Truth)
    │   │   │   │   ├── El Patrón Arquitectónico LLM-as-a-Judge
    │   │   │   │   └── Enfoque Combinado
    │   │   │   ├── Ajuste Fino (Fine-Tuning) de Modelos Fundacionales
    │   │   │   │   ├── Fundamentos y Objetivos de la Especialización
    │   │   │   │   ├── Paradigmas y Enfoques de Fine-Tuning
    │   │   │   │   └── Ingeniería de Datos: Pre-entrenamiento vs. Fine-Tuning
    │   │   │   └── Métricas de Evaluación para Modelos Fundacionales
    │   │   │       ├── ROUGE (Recall-Oriented Understudy for Gisting Evaluation)
    │   │   │       ├── BLEU (Bilingual Evaluation Understudy)
    │   │   │       └── BERTScore (Evaluación Semántica Basada en Tensores)
    │   │   └── EDA y Preprocesamiento
    │   │       ├── Visualización e Interpretación
    │   │       │   ├── 1. Fundamentos de la Visualización
    │   │       │   │   ├── El Reto del Big Data
    │   │       │   │   │   ├── Datos crudos vs. datos modelados
    │   │       │   │   │   └── Riesgo de convertir los datos en un pasivo
    │   │       │   │   ├── Objetivos de Comunicación
    │   │       │   │   │   ├── Contar una historia (Data Storytelling)
    │   │       │   │   │   ├── Facilitar la toma de decisiones
    │   │       │   │   │   └── Ir más allá del dashboard
    │   │       │   │   └── Tipos de Visualización de Datos
    │   │       │   │       ├── Tablas
    │   │       │   │       ├── Gráficos circulares y de barras apiladas
    │   │       │   │       ├── Gráficos de líneas y de áreas
    │   │       │   │       ├── Mapas de calor
    │   │       │   │       ├── Gráficos de dispersión
    │   │       │   │       └── Otras representaciones para patrones, tendencias y anomalías
    │   │       │   ├── 2. Ecosistema de Herramientas de Visualización
    │   │       │   │   ├── QlikView / QlikSense
    │   │       │   │   ├── Power BI
    │   │       │   │   ├── Infogram
    │   │       │   │   ├── FusionCharts
    │   │       │   │   ├── Highcharts
    │   │       │   │   └── Datawrapper
    │   │       │   └── 3. Disciplinas de Interpretación y Minería de Datos
    │   │       │       ├── Tipos de Interpretación
    │   │       │       │   ├── Descriptiva
    │   │       │       │   ├── Inferencial
    │   │       │       │   ├── Comparativa
    │   │       │       │   ├── Causal
    │   │       │       │   └── Predictiva
    │   │       │       └── Técnicas Avanzadas
    │   │       │           ├── Machine Learning
    │   │       │           ├── Procesamiento de Lenguaje Natural (NLP)
    │   │       │           ├── Visualización de Datos Computacional
    │   │       │           ├── Análisis en Tiempo Real
    │   │       │           ├── Análisis Geoespacial
    │   │       │           └── Análisis de Correlaciones
    │   │       └── Limpieza y Optimización en Pandas
    │   │           ├── 1. Estrategias de Imputación para Valores Nulos
    │   │           │   ├── 1.1 Mecanismos de Ausencia
    │   │           │   │   └── Clasificación del patrón de datos faltantes antes de imputar
    │   │           │   └── 1.2 Criterios de Decisión Práctica
    │   │           │       ├── 1.2.1 Imputar con Categoría Centinela ("Unknown")
    │   │           │       ├── 1.2.2 Eliminar el Registro (Complete-case analysis)
    │   │           │       └── 1.2.3 Conservar con Valor Derivado
    │   │           │           ├── Imputación Univariada
    │   │           │           └── Imputación Multivariada
    │   │           ├── 2. Expresiones Regulares (REGEX) en Data Science
    │   │           │   ├── 2.1 El Rol en el Pipeline de Limpieza
    │   │           │   ├── 2.2 Indispensabilidad frente a Métodos Simples
    │   │           │   └── 2.3 Ejemplos de Patrones Comunes
    │   │           └── 3. Tipos de Datos y Eficiencia en Pandas
    │   │               ├── 3.1 Arquitectura de Memoria y Rendimiento Computacional
    │   │               │   ├── Tipos numéricos (int64, float64)
    │   │               │   │   └── Almacenamiento contiguo en NumPy → vectorización y caché de CPU
    │   │               │   └── Tipo object
    │   │               │       ├── Estructura de pointers a objetos Python
    │   │               │       ├── Fragmentación de memoria
    │   │               │       └── Pérdida de vectorización (bucles lentos del intérprete)
    │   │               └── 3.2 La Superioridad del Tipo category en Gran Escala
    │   │                   ├── Dictionary Encoding
    │   │                   │   └── Valores únicos almacenados una sola vez en array de categorías
    │   │                   ├── Sustitución por Pointers Numéricos
    │   │                   │   └── Códigos enteros (int8 / int16) que referencian el diccionario
    │   │                   └── Impacto Técnico
    │   │                       ├── Reducción de consumo de RAM (hasta ~90%)
    │   │                       └── Aceleración algorítmica (groupby, sort_values, value_counts a velocidad C)
    │   │
    │   ├── Ciberseguridad                                          [APLICADO v4 — 44 fragmentos]
    │   │   │
    │   │   ├── 1. Fundamentos de Seguridad
    │   │   │   ├── BLOQUE 1 — Contexto Histórico
    │   │   │   │   ├── Evolución de la seguridad informática (mainframe → Internet → cloud)
    │   │   │   │   └── Del perímetro único a Zero Trust (idea, sin marketing)
    │   │   │   ├── BLOQUE 2 — Contexto Técnico
    │   │   │   │   ├── Triada CIA (Confidentiality, Integrity, Availability)
    │   │   │   │   ├── Amenaza, vulnerabilidad, riesgo, control
    │   │   │   │   └── Superficie de ataque y actores (modelo simple)
    │   │   │   ├── BLOQUE 3 — Arquitectura del Concepto
    │   │   │   │   ├── Defensa en profundidad
    │   │   │   │   ├── Identidad y autenticación vs autorización
    │   │   │   │   └── Logging y detección (rol, no producto SIEM concreto)
    │   │   │   └── BLOQUE 4 — Ejemplo Progresivo
    │   │   │       └── Clasificar un incidente simple con CIA y tipo de control fallido
    │   │   │
    │   │   ├── 2. Criptografía aplicada
    │   │   │   ├── BLOQUE 1 — Contexto Histórico
    │   │   │   │   ├── Cifrado simétrico y asimétrico (línea temporal didáctica)
    │   │   │   │   └── Estándares públicos vs esquemas propietarios opacos
    │   │   │   ├── BLOQUE 2 — Contexto Técnico
    │   │   │   │   ├── Hash, MAC, firma digital (propósitos distintos)
    │   │   │   │   ├── TLS: handshake a nivel conceptual
    │   │   │   │   └── PKI: certificados, CA, validación de cadena
    │   │   │   ├── BLOQUE 3 — Arquitectura del Concepto
    │   │   │   │   ├── Cifrado en tránsito vs en reposo
    │   │   │   │   ├── Gestión de claves (principios; no un KMS de marca)
    │   │   │   │   └── Límites: crypto no sustituye autorización ni input validation
    │   │   │   └── BLOQUE 4 — Ejemplo Progresivo
    │   │   │       └── Por qué HTTPS no corrige un IDOR en la API
    │   │   │
    │   │   ├── 3. Ethical Hacking y Pentesting (marco metodológico)
    │   │   │   ├── BLOQUE 1 — Contexto
    │   │   │   │   ├── Diferencia: pentest, vulnerability assessment, red team (alto nivel)
    │   │   │   │   └── Marco legal y autorización escrita (alcance)
    │   │   │   ├── BLOQUE 2 — Contexto Técnico
    │   │   │   │   ├── Fases típicas: reconocimiento, enumeración, explotación, post-explotación, reporte
    │   │   │   │   └── Inventario y priorización (CVSS como orientación, no verdad absoluta)
    │   │   │   ├── BLOQUE 3 — Arquitectura del Concepto
    │   │   │   │   ├── Reglas de engagement y entornos (prod vs staging)
    │   │   │   │   └── Evidencia y reproducibilidad en el informe
    │   │   │   └── BLOQUE 4 — Ejemplo Progresivo
    │   │   │       └── Del hallazgo técnico a la recomendación accionable
    │   │   │
    │   │   ├── 4. Seguridad en Aplicaciones Web (OWASP)
    │   │   │   ├── BLOQUE 1 — Contexto
    │   │   │   │   ├── OWASP como comunidad y proyecto de referencia
    │   │   │   │   └── OWASP Top 10 como lista orientativa (versiones cambian)
    │   │   │   ├── BLOQUE 2 — Contexto Técnico
    │   │   │   │   ├── Injection (p. ej. SQL) y prepared statements (idea)
    │   │   │   │   ├── Broken Access Control / IDOR
    │   │   │   │   ├── XSS y contextos de salida
    │   │   │   │   └── Security misconfiguration y componentes desactualizados
    │   │   │   ├── BLOQUE 3 — Arquitectura del Concepto
    │   │   │   │   ├── Validación de entrada y encoding de salida
    │   │   │   │   ├── Sesiones, cookies, CSRF (cuándo aplica)
    │   │   │   │   └── API security: autenticación, scopes, rate limiting (conceptos)
    │   │   │   └── BLOQUE 4 — Ejemplo Progresivo
    │   │   │       └── Trazar un flujo login → recurso por ID y puntos de control
    │   │   │
    │   │   └── 5. DevSecOps
    │   │   ├── BLOQUE 1 — Contexto
    │   │   │   ├── Shift-left: seguridad en el ciclo de desarrollo
    │   │   │   └── Relación con CI/CD (sin copiar toda la rama DevOps)
    │   │   ├── BLOQUE 2 — Contexto Técnico
    │   │   │   ├── SAST / DAST / dependency scanning (qué resuelve cada uno)
    │   │   │   ├── Secretos en repositorios (problema y prácticas básicas)
    │   │   │   └── Imágenes de contenedor y superficie de ataque (idea)
    │   │   ├── BLOQUE 3 — Arquitectura del Concepto
    │   │   │   ├── Gates en pipeline (fallar build por hallazgo crítico)
    │   │   │   ├── Política como código (concepto)
    │   │   │   └── Responsabilidad compartida equipo dev / ops / security
    │   │   └── BLOQUE 4 — Ejemplo Progresivo
    │   │       └── Pipeline mínimo: lint → tests → scan dependencias → deploy
    │   ├── DevOps, CI/CD y Automatización
    │   │   ├── Control de Versiones (Git)
    │   │   ├── Contenedores (Docker)
    │   │   ├── Orquestación (Kubernetes)
    │   │   ├── Pipelines CI/CD
    │   │   └── Infrastructure as Code (Terraform, Ansible)
    │   │
    │   └── Arquitectura de Software y Buenas Prácticas   [EN CORPUS V5 — top-up 102 + lotes]
    │       ├── Patrones de Diseño de Sistemas
    │       │   ├── Hexagonal (ports & adapters)
    │       │   ├── CQRS
    │       │   ├── Event-driven (bus, at-least-once, idempotencia)
    │       │   ├── Saga (orquestación / coreografía)
    │       │   ├── Transactional outbox
    │       │   ├── Strangler fig
    │       │   └── BFF (Backend for Frontend)
    │       ├── Principios SOLID, Clean Architecture y DDD
    │       │   ├── Bounded contexts y anti-corruption layer
    │       │   └── Agregados e invariantes
    │       ├── Resiliencia de microservicios
    │       │   ├── Circuit breaker
    │       │   ├── Bulkhead
    │       │   ├── Retry + backoff + jitter
    │       │   ├── Timeouts y deadlines
    │       │   └── Rate limiting (token / leaky bucket)
    │       ├── APIs y contratos
    │       │   ├── Versionado y deprecación
    │       │   ├── Idempotencia
    │       │   ├── Paginación offset / cursor
    │       │   └── Consumer-driven contracts / OpenAPI
    │       ├── Arquitecturas modernas
    │       │   ├── Microservicios vs modular monolith
    │       │   ├── Service mesh (plano de datos)
    │       │   ├── Service discovery
    │       │   ├── EDA / Pub-Sub en OCI (lote fundacional)
    │       │   └── Integración síncrona vs asíncrona / mensajería
    │       ├── Observabilidad
    │       │   ├── Distributed tracing
    │       │   ├── SLI / SLO / error budget
    │       │   └── Métricas RED / USE
    │       ├── Datos en la arquitectura de servicios
    │       │   ├── Consistencia strong vs eventual
    │       │   └── Cache-aside / write-through
    │       ├── Despliegue y multi-tenant
    │       │   ├── Blue/green y canary
    │       │   ├── Feature flags
    │       │   ├── Escalado horizontal y sharding de aplicación
    │       │   └── Multi-tenancy / noisy neighbor
    │       ├── Fiabilidad (chaos engineering controlada)
    │       ├── Trust boundaries y least privilege entre servicios
    │       ├── Clean Code y Refactoring
    │       ├── Testing (Unitario, Integración, E2E)
    │       └── AppSec de aplicación (corpus AppSec residual; ver exclusiones)
    │
    ├── Cloud Computing y Servicios en la Nube
    │   ├── Modelos de Servicio (IaaS, PaaS, SaaS, FaaS)
    │   ├── Oracle Cloud Infrastructure (OCI)
    │   │   ├── Compute, Object Storage, Functions
    │   │   ├── Container Engine (OKE)
    │   │   └── Autonomous Database
    │   ├── Amazon Web Services (AWS)
    │   ├── Microsoft Azure
    │   └── Google Cloud Platform (GCP)
    │
    └── Tecnologías Emergentes
        ├── Internet de las Cosas (IoT) y Edge Computing
        ├── Blockchain y Web3
        ├── Realidad Aumentada / Virtual
        └── 5G y Redes de Nueva Generación

```

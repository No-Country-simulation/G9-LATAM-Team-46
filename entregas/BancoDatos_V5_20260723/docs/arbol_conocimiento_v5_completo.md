# Árbol de conocimientos TechMind — COMPLETO hasta V5

> **Corpus alineado:** `data/augmented/working/dataset_*_v5.csv`  
> **general 530 · train 477** · snapshot aplicado (no reescribir v1–v4)

Este archivo es el **árbol único con todas las ramas fusionadas** hasta la etapa V5.
Los archivos `arbol_v3_rama.md`, `arbol_v4_rama.md` y `arbol_v5_rama.md` documentan
cada ciclo de aumento (fuentes, checklist, mapeo L1); **no** sustituyen este consolidado.

| Etapa | Fuente del subárbol | Qué se integró aquí |
|------:|---------------------|---------------------|
| original / v2 | `arbolaumentado.md` | Árbol base (PDFs + balance de clases) |
| v3 | `arbol_v3_rama.md` §4 | Hardware → *Arquitectura de Computadores y Subsistemas* |
| v4 | `arbol_v4_rama.md` §4 | Software → *Ciberseguridad* (módulos 1–5 + BLOQUEs) |
| v5 | `arbol_v5_rama.md` §4 | IA → *Inteligencia Artificial y sistemas de texto* (módulos 1–6 + BLOQUEs) |

```text
original (PDFs) → v2 balanceo → v3 Hardware arch. → v4 Ciberseguridad → v5 IA texto/ML
```

### Categorías L1 del corpus V5 (`categoria_l1`)
...
> La rama curricular **Ciberseguridad** no es una clase L1 del clasificador: los fragmentos
> se etiquetan como `Redes_y_Comunicaciones` o `Arquitectura` según el foco (ver `arbol_v4_rama.md` §3.1).
...
## Ramas densas por etapa (resumen)

| Etapa | Rama | Fragmentos | Prefijo id | `titulo_origen` |
|------:|------|----------:|------------|-----------------|
| v3 | Arquitectura de Computadores y Subsistemas | 52 | `synth_hw_arch_` | `augment_rama_hardware_arquitectura_v3` |
| v4 | Ciberseguridad | 44 | `synth_cyber_` | `augment_rama_ciberseguridad_v4` |
| v5 | IA y sistemas de texto | 46 | `synth_ia_` | `augment_rama_ia_texto_ml_v5` |

## Ramas aún esqueleto (candidatas v6+)

- Frameworks y Herramientas de Desarrollo (FastAPI / Spring Boot en profundidad)
- DevOps, CI/CD y Automatización (detalle curricular)
- Cloud Computing como rama top-level expandida
- Tecnologías Emergentes
- Lenguajes distintos de Java / Python-Tkinter

## Cómo se usa

| Nivel | Uso |
|-------|-----|
| L1 del CSV | 7 clases de la tabla superior |
| Nodos L2–L4 de este árbol | Vocabulario, keywords, diseño de lotes de aumento |
| BLOQUE 1–4 | Densidad curricular de ramas alimentadas por PDF o sintéticos v3–v5 |

---

*G9-LATAM-Team-46 · TechMind · árbol canónico alineado a dataset V5*

# Árbol de conocimientos — DataSetTechmindV1

**Proyecto:** TechMind AI · G9-LATAM-Team-46 · Hackathon ONE G9  
**Corpus alineado:** `DataSetTechmindV1.csv` (**668** filas)  
**Versión del artefacto:** TechmindV1.0 · 2026-07-23
...
| **L2–L4** | Vocabulario, keywords, diseño de lotes y OCR |
| **BLOQUE / módulos** | Densidad curricular de referencia (no son clases del modelo) |

## Composición del corpus TechmindV1

| Capa | Origen | Filas |
|------|--------|------:|
| PDF nativo (ingesta tipográfica) | vault `data/raw/pdfs` | 343 |
| Sintéticos balanceo | `augment_balanceo_syn_v2` | 38 |
| OCR imagen | `ContieneImagenes/` + Tesseract | 287 |
| **Total** | | **668** |

```text
PDFs nativos (ingesta_v2 tipográfica, 343)
  + balanceo sintético denso (38)     → dataset_general_v2 (381)
  + OCR ContieneImagenes post-proceso (287)
...
**Ratio max/min:** 3.73  
**Longitud:** media 1793 · mediana 1718 · min 147 · max 6396

> **Ciberseguridad** no es L1: material OCR Cisco y AppSec se etiquetan como `Arquitectura` o `Redes_y_Comunicaciones` según el foco.

## Fuentes por capa (resumen)

### A. PDF nativos (tipográficos)
Hardware (lógica digital), Java, Tkinter, Python, SQL, Oracle VM (SO), Redes TCP, Estrategia/Control (Arquitectura), Optimizando FMs / visualización / Pandas (IA).

### B. Balanceo sintético (syn_v2)
- **Sistemas_Operativos:** CFS, vruntime, threads POSIX, cgroups, memoria virtual
- **Bases_de_Datos:** CAP, consistent hashing, Cassandra, compactación
- **Arquitectura:** microservices, EDA, Pub/Sub, functions, resiliencia

### C. OCR (ContieneImagenes)
| PDF / grupo | L1 principal |
|-------------|--------------|
| Conceptos básicos de redes | Redes_y_Comunicaciones |
| Ciberseg Cisco | Arquitectura |
| Curso Python | Lenguajes_Programacion |
| Ciencia datos / IBM / AWS IA / Prompt AWS | Inteligencia_Artificial |
| Agile | Arquitectura |
...
1. Clasificador: **7 clases L1** del CSV.
2. Un fragmento → una categoría dominante.
3. Test set preferible: solo PDF nativos (sin `ocr_*`, sin `augment_*`).
4. TF-IDF: normalización L2; `class_weight=balanced` (ratio ~3.73).
5. Árbol profundo = documentación y expansión; no multiplica clases del MVP.

---

*G9-LATAM-Team-46 · TechMind · árbol alineado a DataSetTechmindV1 · SHA `499fa5a1e122bf3a…`*
